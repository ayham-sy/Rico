...
(localStorage.getItem('mode')) === 'darkmode' ? document.querySelector('body').classList.add('dark') : document.querySelector('body').classList.remove('dark');function darkMode(){localStorage.setItem("mode","darkmode"===localStorage.getItem("mode")?"light":"darkmode"),"darkmode"===localStorage.getItem("mode")?document.querySelector("body").classList.add("dark"):document.querySelector("body").classList.remove("dark")};

(localStorage.getItem('rd')) === 'rdmode' ? document.querySelector('body').classList.add('rd') : document.querySelector('body').classList.remove('rd');function rdMode(){localStorage.setItem("rd","rdmode"===localStorage.getItem("rd")?"rd":"rdmode"),"rdmode"===localStorage.getItem("rd")?document.querySelector("body").classList.add("rd"):document.querySelector("body").classList.remove("rd")};
$(document).ready(function(){
...
var s ="/feeds/posts/default?alt=json-in-script&orderby=updated&start-index=" + n + "&max-results=5";
    $.ajax({
      ...
      success: function (t) {
        ...
        (a += "</ul>"),
          $(".posts-random").each(function () {$(this).html(a);
          });
      },
    });
  });
...
fetch("/feeds/posts/summary?alt=json&max-results=0",{method:"get",headers:{"Content-Type":"application/json"}}).then(e=>e.json()).then(e=>{e.feed.category.forEach(function(e){var r='<div class="caregory-div"><h2 class="Category-sytwd-archivepage">'+e.term+'</h2></div><ul class="clear">';fetch("/feeds/posts/summary/-/"+e.term+"?alt=json&max-results=1000",{method:"get",headers:{"Content-Type":"application/json"}}).then(e=>e.json()).then(e=>{for(var t=0;t<e.feed.entry.length;t+=1){var s=e.feed.entry[t].link.map(function(e){return e.rel}).indexOf("alternate"),a=e.feed.entry[t].link[s].href,s=e.feed.entry[t].title.$t;-1!==a.indexOf(".blogspot.")&&(a=a.replace("http://","https://")),r+="<li><a class='rico-archivepage-posts' title='"+s+"' href='"+a+"'>"+s+"</a></li>"}document.querySelector("#rico-archivepage").innerHTML+="</ul>"+r})})});
$(function(){
